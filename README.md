...
2. Frontend (ReactJS)
Di chuyển vào thư mục frontend (từ thư mục gốc của dự án):
cd [FE_YourEyes] *(Ví dụ: `cd frontend` hoặc `cd client`)* 2.  Cài đặt các thư viện cần thiết:bash npm install *(hoặc `yarn install` nếu bạn dùng Yarn)* 3.  Chạy dự án:bash npm start ``` (hoặc yarn start) 4. Giao diện web sẽ tự động mở và chạy tại: `http://localhost:3000`
